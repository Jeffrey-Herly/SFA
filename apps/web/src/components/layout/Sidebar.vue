...
const navigation = [
  ...
  { name: 'Customers', href: '/customers', icon: 'building-office' },
  { name: 'Activities', href: '/activities', icon: 'clock' },
  { name: 'Users', href: '/users', icon: 'shield-check', roles: ['admin', 'sales_manager'] },
  { name: 'Opportunities', href: '/opportunities', icon: 'chart-bar' },
  { name: 'Orders', href: '/orders', icon: 'shopping-cart' },
  { name: 'Reports', href: '/reports', icon: 'document-text' },
]

const visibleNavigation = computed(() => {
  return navigation.filter(item => !item.roles || (authStore.userRole && item.roles.includes(authStore.userRole)))
})
...
<template>
  <aside 
    class="flex-shrink-0 w-64 bg-white border-r border-slate-200 transition-transform duration-300 ease-in-out hidden md:flex flex-col"
    :class="[isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0']"
  >
    <div class="h-16 flex items-center px-6 border-b border-slate-100">
      <div class="flex items-center gap-2">
        <div class="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center text-white font-bold shadow-lg shadow-blue-500/30">
          SFA
        </div>
    ...
    <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
      ...
      >
        ...
        >
          ...
          <svg v-else-if="item.icon === 'building-office'" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21" />
          </svg>
          <svg v-else-if="item.icon === 'clock'" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <svg v-else-if="item.icon === 'shield-check'" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
    ...
    <div class="p-4 border-t border-slate-100 flex items-center justify-between">
      <div class="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer flex-1 min-w-0">
        <div class="w-9 h-9 rounded-full bg-slate-200 overflow-hidden flex-shrink-0">
          <img :src="`https://ui-avatars.com/api/?name=${encodeURIComponent(authStore.user?.name || 'User')}&background=f1f5f9&color=334155`" alt="User" />
        </div>
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium text-slate-900 truncate">{{ authStore.user?.name || 'Guest' }}</p>
          <p class="text-xs text-slate-500 capitalize truncate">{{ (authStore.user?.role || 'Guest').replace('_', ' ') }}</p>
        </div>
      </div>
      ...
      <button 
        @click="handleLogout"
        title="Sign Out"
        class="p-2 rounded-xl text-slate-400 hover:text-red-600 hover:bg-red-50 transition-all duration-200 flex-shrink-0 ml-1"
      >
